...
def collect_env_info():
	...
	try:
		import MPlexA.detectron2# noqa
		data.append(
			(
				'detectron2',
				MPlexA.detectron2.__version__+' @'+os.path.dirname(MPlexA.detectron2.__file__),
			)
		)
	except ImportError:
	...
	try:
		import MPlexA.detectron2._C as _C
	except ImportError as e:
		data.append(('MPlexA.detectron2._C', f'not built correctly: {e}'))
		...
		if has_cuda and sys.platform!='win32':
			try:
				so_file=importlib.util.find_spec('MPlexA.detectron2._C').origin
...
def main()->None:
	global x
	try:
		from MPlexA.detectron2.utils.collect_env import collect_env_info as f
		print(f())
	except ImportError:
